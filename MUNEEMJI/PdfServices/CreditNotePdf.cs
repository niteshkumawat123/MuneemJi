...
{
    public interface ICreditNotePdf
    {
        Task<string> GetCreditNotePdfById(int id, IWebHostEnvironment _env);
    }

    public class CreditNotePdf : ICreditNotePdf
    {
        string _connectionString = MUNEEMJI.DbConfig.ConnectionString;

        public CreditNotePdf() { }

        public async Task<string> GetCreditNotePdfById(int id, IWebHostEnvironment _env)
        {
            ...
            try
            {
                ...
                using (PdfWriter wri = PdfWriter.GetInstance(doc, stream))
                {
                    ...
                    // ===== SECTION 1: "Credit Note" Title =====
                    var titlePhrase = new Phrase();
                    titlePhrase.Add(new Chunk("Credit Note", new Font(bfArial, 14f, Font.BOLD, darkBrown)));
                    ...
                    string fileName = $"CreditNote_{id}_{DateTime.Now:yyyyMMddHHmmss}.pdf";
