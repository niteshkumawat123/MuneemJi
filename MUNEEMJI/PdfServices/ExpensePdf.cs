...
{
    public interface IExpensePdf
    {
        Task<string> GetExpensePdfById(int id, IWebHostEnvironment _env);
    }

    public class ExpensePdf : IExpensePdf
    {
        string _connectionString = MUNEEMJI.DbConfig.ConnectionString;

        public ExpensePdf() { }

        public async Task<string> GetExpensePdfById(int id, IWebHostEnvironment _env)
        {
            ...
            try
            {
                ...
                using (PdfWriter wri = PdfWriter.GetInstance(doc, stream))
                {
                    ...
                    // ===== SECTION 1: "Expense Voucher" Title =====
                    var titlePhrase = new Phrase();
                    titlePhrase.Add(new Chunk("Expense Voucher", new Font(bfArial, 14f, Font.BOLD, darkBrown)));
                    ...
                    string fileName = $"Expense_{id}_{DateTime.Now:yyyyMMddHHmmss}.pdf";
