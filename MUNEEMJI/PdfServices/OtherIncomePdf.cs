...
{
    public interface IOtherIncomePdf
    {
        Task<string> GetOtherIncomePdfById(int id, IWebHostEnvironment _env);
    }

    public class OtherIncomePdf : IOtherIncomePdf
    {
        string _connectionString = MUNEEMJI.DbConfig.ConnectionString;

        public OtherIncomePdf() { }

        public async Task<string> GetOtherIncomePdfById(int id, IWebHostEnvironment _env)
        {
            ...
            try
            {
                ...
                using (PdfWriter wri = PdfWriter.GetInstance(doc, stream))
                {
                    ...
                    // ===== SECTION 1: "Other Income Receipt" Title =====
                    var titlePhrase = new Phrase();
                    titlePhrase.Add(new Chunk("Other Income Receipt", new Font(bfArial, 14f, Font.BOLD, darkBrown)));
                    ...
                    string fileName = $"OtherIncome_{id}_{DateTime.Now:yyyyMMddHHmmss}.pdf";
