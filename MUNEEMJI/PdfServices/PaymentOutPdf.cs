...
{
    public interface IPaymentOutPdf
    {
        Task<string> GetPaymentOutPdfById(int id, IWebHostEnvironment _env);
    }

    public class PaymentOutPdf : IPaymentOutPdf
    {
        string _connectionString = MUNEEMJI.DbConfig.ConnectionString;

        public PaymentOutPdf() { }

        public async Task<string> GetPaymentOutPdfById(int id, IWebHostEnvironment _env)
        {
            ...
            try
            {
                ...
                using (PdfWriter wri = PdfWriter.GetInstance(doc, stream))
                {
                    ...
                    // ===== SECTION 1: "Payment Voucher" Title =====
                    var titlePhrase = new Phrase();
                    titlePhrase.Add(new Chunk("Payment Voucher", new Font(bfArial, 14f, Font.BOLD, darkBrown)));
                    ...
                    string fileName = $"PaymentOut_{id}_{DateTime.Now:yyyyMMddHHmmss}.pdf";
