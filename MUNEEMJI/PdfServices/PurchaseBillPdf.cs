...
{
    public interface IPurchaseBillPdf
    {
        Task<string> GetPurchaseBillPdfById(int id, IWebHostEnvironment _env);
    }

    public class PurchaseBillPdf : IPurchaseBillPdf
    {
        string _connectionString = MUNEEMJI.DbConfig.ConnectionString;

        public PurchaseBillPdf() { }

        public async Task<string> GetPurchaseBillPdfById(int id, IWebHostEnvironment _env)
        {
            ...
            try
            {
                ...
                using (PdfWriter wri = PdfWriter.GetInstance(doc, stream))
                {
                    ...
                    // ===== SECTION 1: "Purchase Bill" Title =====
                    var titlePhrase = new Phrase();
                    titlePhrase.Add(new Chunk("Purchase Bill", new Font(bfArial, 14f, Font.BOLD, darkBrown)));
                    ...
                    string fileName = $"PurchaseBill_{id}_{DateTime.Now:yyyyMMddHHmmss}.pdf";
