...
{
    public interface IPurchaseReturnPdf
    {
        Task<string> GetPurchaseReturnPdfById(int id, IWebHostEnvironment _env);
    }

    public class PurchaseReturnPdf : IPurchaseReturnPdf
    {
        string _connectionString = MUNEEMJI.DbConfig.ConnectionString;

        public PurchaseReturnPdf() { }

        public async Task<string> GetPurchaseReturnPdfById(int id, IWebHostEnvironment _env)
        {
            ...
            try
            {
                ...
                using (PdfWriter wri = PdfWriter.GetInstance(doc, stream))
                {
                    ...
                    // ===== SECTION 1: "Purchase Return" Title =====
                    var titlePhrase = new Phrase();
                    titlePhrase.Add(new Chunk("Purchase Return", new Font(bfArial, 14f, Font.BOLD, darkBrown)));
                    ...
                    string fileName = $"PurchaseReturn_{id}_{DateTime.Now:yyyyMMddHHmmss}.pdf";
