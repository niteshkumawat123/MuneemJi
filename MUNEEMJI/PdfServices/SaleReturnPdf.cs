...
{
    public interface ISaleReturnPdf
    {
        Task<string> GetSaleReturnPdfById(int id, IWebHostEnvironment _env);
    }

    public class SaleReturnPdf : ISaleReturnPdf
    {
        string _connectionString = MUNEEMJI.DbConfig.ConnectionString;

        public SaleReturnPdf() { }

        public async Task<string> GetSaleReturnPdfById(int id, IWebHostEnvironment _env)
        {
            ...
            try
            {
                ...
                using (PdfWriter wri = PdfWriter.GetInstance(doc, stream))
                {
                    ...
                    // ===== SECTION 1: "Sale Return" Title =====
                    var titlePhrase = new Phrase();
                    titlePhrase.Add(new Chunk("Sale Return", new Font(bfArial, 14f, Font.BOLD, darkBrown)));
                    ...
                    string fileName = $"SaleReturn_{id}_{DateTime.Now:yyyyMMddHHmmss}.pdf";
