...
namespace MUNEEMJI.Repositories
{
    ...
    RETURNING id";
        ...
        public async Task<PurchaseBill?> GetBillByIdAsync(int id)
        {
            ...
            var itemsQuery = @"
     SELECT id, tradedocumentsid, item, quantity, unit, price_per_unit, 
            discount_percentage, discount_amount, tax_amount, tax_percentage, 
            total_amount, itemid, categoryid, serialno, batchno, modelno, 
            expirydate, mfgdate, itemcode, addcessamount, created_on
     FROM TradeDocumentItems 
     WHERE tradedocumentsid = @BillId";
            ...
            while (await itemsReader.ReadAsync())
            {
                bill.BillItems.Add(new PurchaseBillItem
                {
                    ...
                    Item = itemsReader.GetString("item"),
                    Quantity = itemsReader.GetDecimal("quantity"),
                    Unit = itemsReader.IsDBNull("unit") ? null : itemsReader.GetString("unit"),
                    PricePerUnit = itemsReader.GetDecimal("price_per_unit"),
                    ...
                    ExpiryDate = itemsReader.IsDBNull("expirydate") ? (DateTime?)null : itemsReader.GetDateTime("expirydate"),
                    ManufacturingDate = itemsReader.IsDBNull("mfgdate") ? (DateTime?)null : itemsReader.GetDateTime("mfgdate"),
                    ItemCode = itemsReader.IsDBNull("itemcode") ? null : itemsReader.GetString("itemcode"),
                    AddCessAmount = itemsReader.IsDBNull("addcessamount") ? 0 : itemsReader.GetDecimal("addcessamount"),
                    CreatedOn = itemsReader.IsDBNull("created_on") ? DateTime.Now : itemsReader.GetDateTime("created_on")
                });
            }
